	...
	switch(userInput.toUpperCase()) {
		...
		case "V": // View file
			if(state.browse == BROWSE_FILES) {
				...
				if (gFileCnf == null) {
					// Modern Synchronet stores viewers in file.ini [viewer:N]
					// sections, not the legacy file.cnf. Read them directly.
					var _fini = new File(system.ctrl_dir + "file.ini");
					if (_fini.open("r")) {
						var _views = _fini.iniGetAllObjects("key", "viewer:");
						_fini.close();
						for (var _i = 0; _i < _views.length; ++_i) {
							if (_views[_i].ars === undefined) _views[_i].ars = "";
						}
						gFileCnf = { fview: _views };
					}
				}
				if (gFileCnf) {
...
var init = function() {
	...
	{
		// file_area.dir is keyed by lowercase internal codes; normalize so
		// any case passed in on argv[0] resolves correctly.
		if (!file_area.dir[gStartupDirCode]) {
			var upper = gStartupDirCode.toUpperCase();
			for (var c in file_area.dir) {
				if (c.toUpperCase() == upper) { gStartupDirCode = c; break; }
			}
		}
