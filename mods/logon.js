...
if (!(bbs.sys_status&SS_RLOGIN) || options.rlogin_xtrn_logon !== false) {
	if(options.fast_logon !== true || !(bbs.sys_status&SS_FASTLOGON)
		...
		// Serve wide art to large terminals, narrow (img2ans) art to 80-col terminals
		if(console.screen_columns > 80) {
			var wide_files = directory(system.text_dir + "menu/random_wide*");
			if(wide_files.length > 0) {
				var splash = wide_files[random(wide_files.length)];
				bbs.exec("cat " + splash, EX_STDIO | EX_NATIVE);
			}
		} else
