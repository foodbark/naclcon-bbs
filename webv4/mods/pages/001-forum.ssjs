...
// OVERRIDE of stock webv4/pages/001-forum.ssjs. getPagePath() checks
// webv4/mods/pages/ before webv4/pages/, so this file wins.
//
// The ONLY change from stock is one line, marked "foodbark art" below, which
// wraps the message body in fb_art() before it is written.
//
// Why this is server-side rather than client-side: article posts embed
// truecolor half-block ANSI for terminal readers. Stock formatMessage()
// strips the escape sequences but NOT the half-block characters, so ~1600
// raw 0xDF bytes reach the browser as text (they render as a wall of "s"
// with a stroke, being 0xDF read as latin-1). Enabling Synchronet's own
// ANSI-to-HTML path does not help either: html_encode() predates 24-bit
// colour and mis-parses it, leaking fragments like "2;0;40;120m" into the
// page. Doing the substitution here means it cannot be defeated by a stale
// browser cache and can be verified with curl.
//
// If this file ever needs refreshing from upstream, re-copy the stock page
// and re-apply the fb_art() call plus this header.

require('sbbsdefs.js', 'SYS_CLOSED');
load(settings.web_lib + 'forum.js');
load(settings.web_lib + 'avatars.js');

// Replace [art:<name>] ... [/art] with the matching PNG. That image is
// decoded straight out of the same .ans the terminal shows
// (scripts/foodbark_art.py), so both render pixel-identical art.
function fb_art(html) {
	if (!html || html.indexOf('[art:') === -1) return html;
	return html.replace(/\[art:([A-Za-z0-9._-]+)\][\s\S]*?\[\/art\]/g,
		function (whole, name) {
			if (name.indexOf('..') !== -1) return whole;
			return '<img class="fb-art" loading="lazy" ' +
			       'alt="ANSI art: ' + name + '" ' +
			       'src="./images/foodbark/' + name + '.png">';
		});
}
...
if (typeof http_request.query.sub !== 'undefined' &&
	...
	function writeMessage(thread, keys, key, index) {
		...
		// This is a normal message
			...
			// foodbark art: the one line that differs from the stock page.
			writeln(fb_art(formatMessage(body, false, settings.forum_extended_ascii && !header.is_utf8)));
			writeln(strings.message.body.close);
		}
